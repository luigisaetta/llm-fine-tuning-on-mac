...
### Demo 03 — OCI Dedicated AI Cluster endpoint inference
...
### Demos 06, 07, and 08 — ticket-classification training, evaluation, and dataset-shift analysis

[ticket-classification/](ticket-classification/) contains BF16 MPS LoRA training for synthetic IT-support tickets, a separate held-out test evaluation notebook, and deterministic severity dataset-shift analysis. Demo 06 selects the adapter with the greatest validation Category+Severity Accuracy; Demo 07 compares the original base model and saved adapter on held-out category, severity, joint-label, and valid-JSON metrics; Demo 08 compares validation V2 and test-set severity distributions and ticket characteristics to investigate generalization gaps.
...
To retain the configuration generated by the local Transformers installation instead, explicitly disable OCI compatibility:

```bash
python scripts/merge_lora_adapter.py --no-oci-compat
```

## Upload a merged model to OCI Object Storage

Use [`scripts/upload_directory_to_oci_object_storage.py`](scripts/upload_directory_to_oci_object_storage.py) to recursively upload a merged model directory to an existing OCI Object Storage bucket. It uses the OCI Python SDK, handles multi-GiB files with multipart uploads, and shows progress for each file. See the [script usage guide](scripts/README.md) and always run `--dry-run` before a real upload.
