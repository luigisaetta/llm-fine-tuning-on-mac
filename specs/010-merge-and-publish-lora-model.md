...
Provide a command-line script that loads the local `Qwen3-1.7B` base model and a local LoRA adapter, merges the adapter into the base weights, saves a standalone Safetensors model and tokenizer under an ignored artifact directory, and can upload that directory to a model repository on the Hugging Face Hub.
