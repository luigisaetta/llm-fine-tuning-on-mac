...
The future implementation will add a restartable training notebook and supporting documentation under `ticket-classification/`. It will reuse the Qwen3-1.7B local model and the BF16 MPS LoRA approach of Demo 01, but use the ticket-classification datasets and evaluation protocol defined here.
...
| --- | --- | --- | ---: |
| Training | `ticket-classification/artifacts/datasets/train_dataset.jsonl` | Fit LoRA parameters. | 1,000 |
| Validation | `ticket-classification/artifacts/datasets/validation_dataset.jsonl` | Evaluate loss after every epoch and select the best checkpoint. | 200 |
...
Each JSONL record has a `messages` array with exactly a `user` ticket message followed by an `assistant` message. The assistant content must parse as one JSON object with exactly the keys `category`, `severity`, and `summary`. The notebook must reject empty ticket text, empty output values, invalid JSON, extra or missing keys, duplicate records within a split, and an exact ticket-text overlap between any two splits.
...
* Use `eval_strategy="epoch"`, `save_strategy="epoch"`, `load_best_model_at_end=True`, and `metric_for_best_model="eval_loss"` with `greater_is_better=False`. The validation split is the only split passed to the trainer.
* Use deterministic validation and test generation (`do_sample=False`) with the Qwen chat template and thinking disabled.
...
* Save checkpoints, logs, tokenizer, adapter, configuration, and training metadata only below `ticket-classification/artifacts/fine-tuned-models/`. Save an adapter, never a duplicate base model.

## Held-out test evaluation contract

The test split must not influence training, checkpoint selection, early stopping, hyperparameter selection, or prompt changes. After training restores the best validation-loss adapter, evaluate every test record once and report:
...
* exact structured-response rate; and
* mean token F1 for the `summary` field, using the same lowercase bag-of-token F1 definition as Demo 01.

For all label and exact-response metrics, an invalid response counts as incorrect. Report the validation loss for every epoch separately from final test metrics. Show aggregate numbers only by default; optional examples must be explicitly capped and must not be stored in the notebook.
...
* The configuration and actual device, dtype, dataset-size, and trainable-parameter information are visible before training begins.
* Validation loss runs at every epoch and the best adapter is restored by validation loss.
* The test split is used only once after training and produces every metric in the held-out test evaluation contract.
